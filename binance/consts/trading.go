...
const (
	...
	// ApiOrderTest 测试下单接口 (TRADE),用于测试订单请求，但不会提交到撮合引擎
	...
	// ApiCancelReplace 撤消挂单再下单 (TRADE)
	//撤消挂单并在同个交易对上重新下单。
	//在撤消订单和下单前会判断: 1) 过滤器参数, 以及 2) 目前下单数量。
	//即使请求中没有尝试发送新订单，比如(newOrderResult: NOT_ATTEMPTED)，下单的数量仍然会加1。
	ApiCancelReplace = "/api/v3/order/cancelReplace"

	// ApiAllOrders 查询所有订单（包括历史订单） (USER_DATA)
	ApiAllOrders = "/api/v3/allOrders"
	...
	// ApiOrderListOCO 发送新 one-cancels-the-other (OCO) 订单，激活其中一个订单会立即取消另一个订单。
	...
	ApiOrderListOCO = "/api/v3/orderList/oco"

	// ApiOrderListOTO 发送一个新的 OTO 订单。
	...
	ApiOrderListOTO = "/api/v3/orderList/oto"

	// ApiOrderListOTOCO 发送一个新的 OTOCO 订单。
	...
	ApiOrderListOTOCO = "/api/v3/orderList/otoco"

	// ApiAllOrderList 查询所有订单列表 (USER_DATA)
	//根据提供的可选参数检索所有的订单列表。
	//请注意，startTime和endTime之间的时间不能超过 24 小时。
	ApiAllOrderList = "/api/v3/allOrderList"

	// ApiOpenOrderList 查询订单列表挂单 (USER_DATA)
	ApiOpenOrderList = "/api/v3/openOrderList"
)
